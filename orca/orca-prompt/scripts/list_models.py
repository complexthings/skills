...
Usage:
    python3 list_models.py <pi|prime-agent|opencode|agy> [orchestrationWorker|subagent|subagentSimple]
...
`data/model-priority.json` instead of alphabetically: ranked models first in
...
SLOTS = ("orchestrationWorker", "subagent", "subagentSimple")
...
def load_priority(path=PRIORITY_PATH):
    """Read `data/model-priority.json` — the single source of both orderings."""
...
def main(argv):
    if not 2 <= len(argv) <= 3 or argv[1] not in COMMANDS or (len(argv) == 3 and argv[2] not in SLOTS):
        sys.stderr.write("usage: list_models.py <%s> [%s]\n" % ("|".join(COMMANDS), "|".join(SLOTS)))
        return 2
    harness = argv[1]
    ...
    try:
        report = list_models(harness, result.stdout, slot)
